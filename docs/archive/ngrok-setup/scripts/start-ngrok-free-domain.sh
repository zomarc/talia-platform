...
# Start ngrok with the free domain (while waiting for DNS propagation)
...
FREE_DOMAIN="kelley-resistless-sniffingly.ngrok-free.dev"
FRONTEND_PORT=5173
BASE_PATH="/celestyal"

echo -e "${BLUE}🌐 Starting ngrok with Free Domain${NC}"
echo "=========================================="
echo -e "${YELLOW}📋 Configuration:${NC}"
echo "   • Domain: ${FREE_DOMAIN}"
...
ngrok http ${FRONTEND_PORT} --domain=${FREE_DOMAIN} > /tmp/ngrok-free-domain.log 2>&1 &
NGROK_PID=$!
...
if kill -0 $NGROK_PID 2>/dev/null; then
    ...
    if [ "$PUBLIC_URL" != "N/A" ] && [ -n "$PUBLIC_URL" ]; then
        ...
        echo "   • This is a temporary URL while DNS propagates"
        echo "   • Once DNS is ready, switch to: ./scripts/start-ngrok-celestyal.sh"
    ...
    echo "   Check log: /tmp/ngrok-free-domain.log"
    cat /tmp/ngrok-free-domain.log | tail -10
    exit 1
fi
